...
const TableUserAccount = () => {
    ...
    return {
      ...
      option: (
        <OptionButtonMenuTable
          setAndOpen={() => openAndSetIds(result.uId)}
          setAndDelete={() => openAndSetIdsDelete(result.uId)}
        />
      ),
    };
  });
  ...
  const updateUserAccount = async (ids, values) => {
    try {
      const config = {
        headers: {
      ...
      const result = await instance.put(`/user/update/id/${ids}`, body, config);
      toast({
        title: "Berhasil",
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm, setSubmitting }) => {
      await updateUserAccount(ids, values);
      resetForm({});
      onClose();
    },
  ...
  const deleteData = async () => {
    try {
      const result = await instance.delete(`/user/delete/id/${ids}`);
      toast({
        title: "Berhasil",
